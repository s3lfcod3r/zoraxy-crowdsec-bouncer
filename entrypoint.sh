...
# config.yaml in den Plugin-Ordner schreiben
cat > "$PLUGIN_DEST/config.yaml" <<YAML
...
# Plugin-Pfad explizit übergeben damit Zoraxy das Plugin findet
# und das Plugin aus seinem eigenen Verzeichnis gestartet wird
exec zoraxy -docker=true -port=:8000 -plugin=/opt/zoraxy/plugin
